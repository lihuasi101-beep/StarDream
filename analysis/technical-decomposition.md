...
本轮兼容层故障修复补齐了两条运行时边界。浮标竖向漂移的根因是透明输入层单独按自身矩形归一化，而 js-dos Canvas 还按后端原生帧尺寸做宽高比适配；窗口缩放、全屏或运行库重排后，两套矩形会出现线性差异。现在以 Canvas 的 `getBoundingClientRect()` 和 `CommandInterface.width()/height()` 计算唯一的 aspect-fit 输入视口，并让输入层镜像 Canvas 的最终矩形。读档卡住的根因是历史列表、最新记录、虚拟 `STARSAVE.SSS` 读取和 `player.stop()` 原先都可能无限等待，且旧运行实例的异步事件可能覆盖新实例状态。现在存储读取、虚拟文件读取和播放器停止均有超时；读档切换先递增运行代次使旧事件失效，再清理旧页面状态并启动选中存档，IndexedDB 不可用时回退到同步刷新镜像。
