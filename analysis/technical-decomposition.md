...
存档槽丢失的根因是旧实现只持久化 `STARSAVE.SSS`，而原版启动器和主程序还会读取 `STARSA00.SSS` 及运行中生成的 `STARSAxx.SSS` 槽文件。页面刷新后虽然主存档可以注入并进入游戏，但辅助槽文件仍被发行包里的初始文件覆盖，原版“读取进度”菜单因此看不到之前的本地槽位。现在通过 `CommandInterface.fsTree()` 枚举当前虚拟盘，捕获全部 `STARSAVE.SSS`/`STARSA*.SSS` 文件，按文件名、大小和字节内容存入 IndexedDB v3 与刷新镜像；下次启动或外挂存档读档时完整恢复这组文件，并兼容旧版只有主文件的记录。
...
原版秘籍入口已增加网页按钮。按钮要求在原版公司界面使用：在 `ci-ready` 后发送 js-dos 键码 `341` 按住 Ctrl，以真实按下/保持/释放节奏发送 ASCII `A`、`L`、`E`、`X`，先释放 Ctrl，再单独发送 `8`；原版编辑界面仍负责实际修改股票价格，网页只提供可视化触发入口并记录 Ctrl 释放与逐键发送结果。
